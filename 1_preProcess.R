...
# 2.process the 'children' column to create a list of vectors, convert this to a data frame
...
# 3.use melt to establish child-parent relations
...
# 4.stop if there is not a unique parent for every child
...
# 5.construct tree data (root and level)
...
# 10.check that no news rows were created with join
stopifnot(nrow(object_enhanced) == length(unique(object_enhanced$module_id)))
